/**
 * @file oled.c
 * @author enwillcn@163.com
 * @brief 修改自淘宝卖家提供的库，改为 SPI+DMA 刷屏
 * @version 0.1
 * @date 2023-01-05
 * 
...
extern uint32_t l_fb[BSP_SCREEN_HEIGHT + 1][BSP_SCREEN_WIDTH / 32U];
...
{	
	...
	if(cmd)
	  ...
	  OLED_DC_Clr();		  
	OLED_CS_Clr();				
	HAL_SPI_Transmit(&SPI_PORT,&dat,1,100); 		  
	OLED_CS_Set();
	OLED_DC_Set();   	  
}
...
void OLED_Refresh(void)
{
	...
	for(i=0;i<8;i++)
	{
	   ...
	   for(n=0;n<130;n++)									//如果右侧有不能控制的竖线，这里由128改为130，然后调用清屏函数
		 ...
		 HAL_SPI_Transmit(&SPI_PORT,&OLED_GRAM[n][i],1,100);
  }
}
...
{	
	...
	for (y=0; y<BSP_SCREEN_HEIGHT; y++)	{
		for (x=0; x<BSP_SCREEN_WIDTH/32U; x++) {
			for (n = 0; n < 32; n++) {
				if (l_fb[y][x] & (1<<n)) {
					OLED_GRAM[y/8U][x*32+n+2 ] |= (1<<(y%8U));
				}
				else {
					OLED_GRAM[y/8U][x*32+n+2 ] &= ~(1<<(y%8U));
				}
	...
	OLED_CS_Clr();
	OLED_SDA_Set();
	HAL_SPI_Transmit_DMA(&SPI_PORT,&OLED_GRAM[dma_counter][0],132);
    dma_busy = 1;
}
//清屏函数
