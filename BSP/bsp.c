...
uint32_t l_fb[BSP_SCREEN_HEIGHT + 1][BSP_SCREEN_WIDTH / 32U];
 	...
 	if(htim == &htim3)
   	{
		...
		//QTIMEEVT_TICK_X(0U, 0); /* process time events for rate 0 */
		QACTIVE_POST(the_Ticker0, 0, 0); /* post a don't-care event to Ticker0 */
		QACTIVE_PUBLISH(&tickEvt,0); /* publish to all subscribers */
        BSP_buttonPress();
   	}
 }    
...
void BSP_buttonPress(void)
{
    ...
    current = ~BTN2_GPIO_Port->IDR;
    tmp = buttons.depressed;
    buttons.depressed |= (buttons.previous & current);  // 只有current 和 previous 同时为1 时才确认为depressed
    buttons.depressed &= (buttons.previous | current);  // 只有current 和 previous 同时为0 时才确认为released
    buttons.previous = current;
    tmp ^= buttons.depressed;                           // 状态有变化的buttons相应的位会被置1
    if ((tmp & BTN2_Pin) != 0U){                  // 状态有变化即判断为真
        if(buttons.depressed & BTN2_Pin){                   // 判断是按下
            /* post the "button-pressed" event from ISR */
            static QEvt const trigEvt = { PLAYER_TRIGGER_SIG, 0U, 0U};
            QACTIVE_PUBLISH(&trigEvt, 0);
        }
    }
...
bool BSP_isThrottle(void) { /* is the throttle button depressed? */
    ...
    return (((BTN1_GPIO_Port->IDR) & BTN1_Pin) == 0U);
}
/*..........................................................................*/
...
void BSP_updateScore(uint16_t score) {
    ...
    if (score == 0U) {
        BSP_paintString(1U, BSP_SCREEN_HEIGHT - 7U, "SCORE:");
    }
    ...
    str[1] = (s % 10U) + '0'; s /= 10U;
    str[0] = (s % 10U) + '0';
    BSP_paintString(6U*6U, BSP_SCREEN_HEIGHT - 7U, str);
